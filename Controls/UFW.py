...
import subprocess
import time
from LOGGING import log_changes
...
def ensure_ufw_installed():
    ...
    if not is_ufw_installed():
        var = input("This point onwards, the configurations require the installation of UFW. Do you want to install the Host firewall? (yes/no):").lower()
        var.lower()
...
def ensure_iptables_persistent_packages_removed():
    ...
    if is_iptables_persistent_installed():
        var = input("\nDo you want to remove the iptables-persistent packages? (yes/no):").lower()
        var.lower()
...
def enable_firewall_sequence():
    ...
    if not is_ufw_enabled():
        ...
        if var == 'y' or var == 'yes' or var == '':
            ...
            line="""UFW-ENABLING: ok, below commands were executed:
